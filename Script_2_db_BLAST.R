...
system("tblastx -query PPO_mango_gene.fasta -db chirimoyaCDS_db -out resultados_tblastx_e-5.txt -evalue 1e-5 -outfmt 6")

# Leer resultados
results <- read.table("resultados_tblastx_e-5.txt", header = FALSE)
head(results)
...
df_sin_duplicados  <- results %>%
  filter(query_id != subject_id, `%identity` > 70)
...
write.table(genes_homologos_chirimoya, file = "genes_homolo_chirimoya_PPO.txt", 
